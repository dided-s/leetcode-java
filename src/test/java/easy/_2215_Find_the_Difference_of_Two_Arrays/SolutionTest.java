...
public class SolutionTest {
    ...
    void testArguments(int[] nums1, int[] nums2, List<List<Integer>> expected) {
        List<List<Integer>> actual = new Solution().findDifference(nums1, nums2);

        Assertions.assertEquals(expected.size(), actual.size());
        for (int i = 0; i < actual.size(); i++) {
            Assertions.assertEquals(Set.of(expected.get(i)), Set.of(actual.get(i)));
        }
    }
